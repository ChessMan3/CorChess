...
namespace {
  ...
  int remaining(int myTime, int myInc, int moveOverhead, int movesToGo, int ply)
  {
    ...
    else
    {
        ...
        sd = 1.0 + 17.0 * mn / (500.0 + mn);
        TRatio = (T == OptimumTime ? 0.0196 : 0.0743) * sd;
    }
    
    /// In the case of no increment we simply have ratio = std::min(1.0, TRatio); The usage of increment follows a normal distribution with the maximum around move 23.
    
    double incUsage = 48.9 + 53.0 * gauss(mn, 23.1, 531.0);
